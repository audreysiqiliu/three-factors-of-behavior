...
# Filter out UserIds with any TrialsSinceLast_Illegal1Name_ByDay or TrialsSinceLast_target_present_ByDay greater than 23
user_ids_to_exclude = df_cleaned[df_cleaned['TrialsSinceLast_Illegal1Name_ByDay'] > 23]['UserId'].unique()
user_ids_to_exclude = user_ids_to_exclude.union(df_cleaned[df_cleaned['TrialsSinceLast_target_present_ByDay'] > 23]['UserId'].unique())

# Apply the filter to remove those UserIds
df_cleaned = df_cleaned[~df_cleaned['UserId'].isin(user_ids_to_exclude)].copy()

trial_numbers = df_cleaned['TrialNumber'].unique()
print(f"Unique TrialNumber values in df_cleaned:")
print(sorted(trial_numbers))
print(f"Total unique TrialNumbers: {len(trial_numbers)}\n")

values = df_cleaned['TrialsSinceLast_target_present_ByDay'].unique()
print(f"Unique values of TrialsSinceLast_target_present_ByDay:")
print(sorted(values))
print(f"Total unique TrialsSinceLast_target_present_ByDay: {len(values)}\n")

values2 = df_cleaned['TrialsSinceLast_Illegal1Name_ByDay'].unique()
print(f"Unique values of TrialsSinceLast_Illegal1Name_ByDay:")
print(sorted(values2))
print(f"Total unique TrialsSinceLast_Illegal1Name_ByDay: {len(values2)}\n")

# Save the filtered DataFrame to the output path as df_cleaned_test
df_cleaned.to_csv(f'{output_path}/df_cleaned_test.csv', index=False)
